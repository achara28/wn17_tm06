...
<div  class="container" style="font-family:'ComicSans MS', cursive, sans-serif">
	...
	<div id="caruso"  style="visibility:hidden; font-size:20px; color:#4c4c4c;">Students have <strong><span id="time"> 00:30</span></strong> seconds to answer the question <strong><span id="qid"></span></strong></div>
	...
	<div class="container" style="overflow-y:scroll; height: 100%; max-height: 500px;">
		...
		<div class="row">
			<div class="col-sm-5" style="border-right: solid 1px; border-right-color:#f2f2f2; word-wrap: break-word; ">
				<h3>  Question {{$sum}}   </h3>
				<br> <br>
				<p style="font-size: 17px">{{$question->question}} </p>
				<br> <br> 


				<div class="modal fade" id="modal{{$sum}}" role="dialog">
					<div class="modal-dialog">
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								<form method="POST" action="/user/updatequestion/{{ $question->id }}">
									{{ csrf_field() }}
									...
									<div class="form-group">
										...
										<input type="text" class="form-control" id="answer1" name="answer1" required="true" value="{{$question->answer1}}">
									...
									<div class="form-group">
										...
										<br> <br>
										<input id="corr1{{$sum}}" type="radio" name="correct" value="answer1" required > <span style="color:#f9b754;" >Answer 1</span> <br>
										<input  id="corr2{{$sum}}" type="radio" name="correct" value="answer2" required> <span style="color:#296cc6;" >Answer 2</span> <br>
										<input  id="corr3{{$sum}}" type="radio" name="correct" value="answer3" required> <span style="color:#ce3d3d;" >Answer 3</span> <br>
										<input  id="corr4{{$sum}}" type="radio" name="correct" value="answer4" required> <span style="color:#60a850;" >Answer 4</span> <br>
									</div>
									<br>
				...
				<script type="text/javascript">
					...
					if (corr==="answer1")
						document.getElementById("corr1{{$sum}}").checked = true;
					else if (corr==="answer2")
						document.getElementById("corr2{{$sum}}").checked = true;
					else if ((corr==="answer3"))
						document.getElementById("corr3{{$sum}}").checked = true;
					else
						document.getElementById("corr4{{$sum}}").checked = true;
				</script>
				...
				<div class="row">
					<div class="col-sm-12 ">
						...
						class="btn btn-warning btn-md center-block lucianoedit" data-toggle="modal" data-target="#modal{{$sum}} " Style="width: 100px;" OnClick="btnSearch_Click" >Edit</button> 




						<form method="post"  id="btnClear" action="/question/delete/{{ $question->id }}" style="text-align: center;">
							{{csrf_field() }}  
							{{ method_field('DELETE') }}
							<button Style="width: 100px;" type="submit" class="btn btn-danger lucianodelete">Delete</button>
						</form>
{{-- 
				<form method="post"  id="btnShow" action="/question/{{ $question->id }}" style="text-align: center;">
					{{csrf_field() }}   --}}
					<button Style="width: 100px;" type="submit" onclick="start({{ $question->id }},{{$sum}} )" class="luciano btn btn-success">Project</button>
					{{-- 	</form> --}}
				</div>
			</div>
		</div>
		<div style="color: #4c4c4c" class="col-sm-7">
			@if (($question->anleo))
			<h3>Answers </h3>		
			<p> <strong style="color: #f9b754"> Answer 1 </strong> : <span style="font-size: 17px">{{$question->no1}}</span></p>
			<p><strong style="color: #296cc6"> Answer 2 </strong> : <span style="font-size: 17px">{{$question->no2}}</span></p>
			<p><strong style="color: #ce3d3d"> Answer 3 </strong> : <span style="font-size: 17px">{{$question->no3}}</span></p>
			<p><strong style="color: #60a850"> Answer 4 </strong> : <span style="font-size: 17px">{{$question->no4}}</span></p>
			<p><strong style="color:"> Correct Answer </strong> : <span style="font-size: 17px">{{$question->cor}}</span></p>
