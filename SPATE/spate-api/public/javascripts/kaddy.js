...
$(function () {
    ...
    var s = 0;
    var m = 0;
    $.getJSON('/assets/data/pythia.json', function (data) {
        var apijson = data;
      ...
      ' + globalparams + '\
        ...
        $('.try').on('click', function (e) {
            ...
            $.each($(this).prev('table').find('.parameter'), function (k, v) {
                if ($('.name', v).html() == "files") {
                    if ($('input,select', v).eq(1).val() != null) {
                        post = true;
                        //console.log("test");
                        //console.log($('input,select', v).eq(1));
                        //console.log($('input,select', v)[1].files[0]);
                        return;
                    }
                }
            });
            if (!post) {
                ...
                if ($(self).closest('.accordion-inner').find('pre').length == 0) {
                    $(self).closest('.accordion-inner').append('<pre class="prettyprint request-url"><h5>Request URL</h5><p></p></pre><pre class="prettyprint response-headers"><h5>Response Headers</h5><p></p></pre><pre class="prettyprint  response-body pre-scrollable"><h5>Response Body</h5><p></p></pre><pre class="prettyprint response-time"><h5>Response Time</h5><p></p></pre>');
                } else {
                ...
                if ($(self).closest('.method').find('.verb').html() == 'IMAGE') {
                    ...
                    geturl = $.ajax({
                        ...
                        error: function (data, status, XHR) {
                            ...
                            $(self).closest('.accordion-inner').find('.response-time p').html("You waited: " + totalTime + " ms");
                ...
                $.each($(this).prev('table').find('.parameter'), function (k, v) {
                    ...
                    if ($('.name', v).html() == "files") {
                        if ($('input,select', v).eq(1).val() != null) {
                            ...
                            //console.log($('input,select', v)[1].files[0]);
                            //console.log($('input,select', v)[1].files[0]);
                            params.append("file", $('input,select', v)[1].files[0]);
                        }
                    }
                    else {
                        //alert("here2");
                        //console.log($('input,select', v).val());
                        params.append($('.name', v).html(), $('input,select', v).val());
                    }
                });
//                var matches = reqURL.match(/(:(?:[a-z][a-z0-9_-]*))/g);
//                if (matches) {
